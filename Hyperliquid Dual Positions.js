...
async function createPositionStack(stack, position) {
  ...
  const direction = getPositionDirection(unrealizedPnl, entryPx, currentPrice)
  ...
  createCenteredText(overlay, `${coin}${direction ? ' ' + direction : ''}`, 
    direction === "Long" ? CONFIG.COLORS.POSITIVE : 
    direction === "Short" ? CONFIG.COLORS.NEGATIVE : 
    CONFIG.COLORS.TEXT, CONFIG.FONTS.POSITION)
